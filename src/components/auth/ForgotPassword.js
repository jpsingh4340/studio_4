...
export default function ForgotPassword() {
  ...
  const handleEmailChange = e => {
    ...
    setValidation(val);
    setSuggestions(
      value.includes("@") && !val.isValid ? suggestEmails(value) : [],
    );
        ...
        {success ? (
          <div className="text-center">
            ...
            <ul className="list-unstyled text-start small text-muted mt-3">
              <li className="mb-1">
                <i className="bi bi-exclamation-circle me-1" /> Check
                spam/junk folder
              </li>
              <li>
                <i className="bi bi-clock me-1" /> Link expires in 15 minutes
              </li>
            </ul>
            ...
            {resendAttempts > 0 && (
              <p className="mt-3 text-muted small">
                Email sent {resendAttempts}{" "}
                {resendAttempts === 1 ? "time" : "times"}
              </p>
            )}
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="position-relative">
            ...
            <div className="position-relative mb-3">
              ...
              {validation.message && (
                <div
                  className={`form-text ${validation.isValid ? "text-success" : "text-danger"}`}
                >
                  {validation.message}
                </div>
              )}
            ...
            {suggestions.length > 0 && (
              <ul
                className="list-group position-absolute w-100 z-10 border rounded shadow-sm bg-white mt-n2"
                style={{ top: "70px", zIndex: 10 }}
              >
                <li className="list-group-item small fw-medium text-muted">
                  Did you mean?
                </li>
